...
class TestSequence:
    ...
    def test_writeread(self, seq_func, tmp_path, compare_seq_file):
        ...
        assert seq2.calculate_kspace(output_dict=True) == Approx(seq.calculate_kspace(output_dict=True), abs=1e-1, nan_ok=True)
    ...
    def test_recreate(self, seq_func):  # noqa: ARG002
        ...
        # Test for approximate equality of kspace calculation
        assert seq2.calculate_kspace(output_dict=True) == Approx(seq.calculate_kspace(output_dict=True), abs=1e-6, nan_ok=True)
